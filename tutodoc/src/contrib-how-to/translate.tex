...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		Dans ce cas, on utilisera le \mailsubject{en FOR italian} avec le nom anglais de la langue, ceci afin de préserver la santé mentale de l'auteur de \thisproj.
	\end{enumerate}


	\item Le dossier \verb#en# doit ensuite être renommé en \verb#italian# avant de faire les traductions.%
	\footnote{
		Comme indiqué plus haut, il n'y a pas de besoin réel du côté du dossier \texttt{doc} de la documentation.
	}


	\item Pour livrer le travail fini, deux options s'offrent à vous.
	%
	\begin{enumerate}
		\item L'idéal est de passer via \thisrepo\ en usant d'un classique \verb#git push#\,.

		\item En cas d'allergie connue à \git\,, vous pourrez envoyer votre dossier de traductions compressé au format \verb#zip# ou \verb#rar# via un mail de \mailsubject{italian}\,.
	\end{enumerate}
\end{enumerate}
...
\begin{tdocimp}
  ...
  in_EN = #1 in english\end{verbatim}
	...
	\tdocsep
  	...
  	Les traductions se font après chaque signe \,$=$\, sans toucher à ce qui se trouve avant qui est utilisé en interne par le code de \thisproj.
\end{tdocimp}
