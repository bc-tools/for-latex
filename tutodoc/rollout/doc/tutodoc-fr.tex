...
\begin{tdocfix}
	\item Mise en avant colorée de contenus : pour les \tdocmacro{newkeytheorem} utilisés avec le thème \tdocinlatex{draft}, il a fallu ajouter \tdocinlatex{postheadhook = \leavevmode} (ceci est nécessaire car le contenu peut juste être de type liste).
\end{tdocfix}
...
\begin{tdocnew}
	...
	\begin{enumerate}
		...
		\item Chaque environnement dispose d'une nouvelle option \tdocinlatex{col} pour la couleur du contenu indiquant les changements.
	\end{enumerate}
\end{tdocnew}
...
La classe \tdoccls{scrartcl} est chargée via l'option \tdocinlatex{fontsize = 10pt}\,, et le package \tdocpack{geometry} gère les dimensions de la page.
...
\begin{tasks}[label=\small\textbullet](3)
    \task \tdocinlatex|en| : anglais.
        \task \tdocinlatex|es| : espagnol.
        \task \tdocinlatex|fr| : français.
\end{tasks}
...
\begin{tdocnote}
    ...
    \footnote{
        Par exemple, les thèmes \tdocinlatex{bw} et \tdocinlatex{draft} ne tiennent pas compte de la clé \tdocinlatex{col-stripe} !
    }
    %
...
\begin{tdocexa}
    ...
    Via \tdocinlatex+\tdocshowcaseinput[col-stripe = red, col-text = orange!75!black]{external.tex}+\,, on obtient le changement de couleur observable ci-après.
...
\begin{tdocexa}[Mise en situation]
    \tdocinlatex+\tdoclatexshow{examples-listing-xyz.tex}+ produit ce qui suit.
...
Les clés optionnelles \tdocinlatex{col-chges}\,, \tdocinlatex{date} et \tdocinlatex{version} permettent de dater et versionner directement un changement d'un type particulier. Voici des exemples d'utilisation.
...
\begin{tdocbreak}[version = 1.6.0, date = 2024-10-27]
    \item L'environnement \tdocenv*{showcase} et ses descendants : la clé \tdocinlatex{color} a été renommée \tdocinlatex{col-stripe}.
...
\begin{tdocnew}
    ...
    \item L'environnement \tdocenv*{showcase} et ses descendants : la clé \tdocinlatex{col-text} permet de changer aussi la couleur du texte.
