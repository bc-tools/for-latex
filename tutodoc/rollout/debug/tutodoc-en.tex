...
\begin{tdocbreak}
	\item Format: the \tdoccls{scrartcl} class replaces the venerable \tdoccls{article}. This implies better placement of the margins with the options retained for loading \tdoccls{scrartcl}.
...
\begin{tdocnew}
	...
	\begin{enumerate}
		...
		\item Each environment has a new option \tdocinlatex{col} for the color of the material indicating changes.
	\end{enumerate}
\end{tdocnew}
...
\begin{tdoctech}
	...
	%
	\begin{enumerate}
		\item \texttt{CSS}-like: one file per theme with a name like \texttt{tutodoc-bw.css.cls}\,.
...
On the other hand, the \thisproj\ class identifies \tdocinlatex|en| as the main language used by \tdocpack{babel}.%.
\footnote{
...
\begin{tdocexa}
    ...
    As for \tdocinlatex+\tdocshowcaseinput[col-stripe = red, col-text = orange!75!black]{external.tex}+\,, this will produce the color change shown below.
...
\begin{tdocbreak}[version = 1.6.0, date = 2024-10-27]
    \item The \tdocenv*{showcase} environment and its descendants: the \tdocinlatex{color} key has been renamed \tdocinlatex{col-stripe}.
...
\begin{tdocnew}
    ...
    \item The \tdocenv*{showcase} environment and its descendants: the \tdocinlatex{col-text} key can also be used to change the text color.
