...
\begin{bdoctopic}[Contenus mis en avant]
    ...
    \begin{itemize}
        \item \bdocenv{bdocwarning} permet d'avertir,

        \item \bdocenv{bdocinfo} sert à donner des informations utiles à connaître.
    \end{itemize}
\end{bdoctopic}
...
\begin{bdoctopic}<Paramètres locaux>
    \begin{itemize}
        ...
        \item Si le package \bdocpack{babel} est employé avec la langue \bdocinlatex{french} comme langue principale, l'espacement autour des doubles-points sera celui proposé par \bdocpack{babel}.

        \item Les guillemets sont gérés via le package \bdocpack{csquotes} afin d'obtenir un résultat \bdocquote{professionnel}.
    \end{itemize}
\end{bdoctopic}
